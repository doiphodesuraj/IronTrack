    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="max-w-full break-words text-4xl sm:text-6xl lg:text-7xl font-black uppercase tracking-tighter leading-tight text-white">
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            <Play size={20} className="relative z-10" /> <span className="relative z-10 font-black">QUICK START</span>
          </button>
          <Link to="/templates" className="glass-btn bg-neon-purple/20 hover:bg-neon-purple/30 text-white border border-neon-purple/30 group">
            <Plus size={20} className="group-hover:rotate-90 transition-transform duration-500" /> <span className="font-black">NEW TEMPLATE</span>
          </Link>
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <div key={i} className="glass-card p-8 float-3d group">
            ...
            <p className="text-6xl font-black font-mono tracking-tighter">{stat.value}</p>
          </div>
        ))}
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              recentSessions.map(session => (
                <div key={session.id} className="glass-card p-6 flex items-center justify-between group hover:bg-white/5 transition-all float-3d">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h3 className="text-xl font-black uppercase tracking-tight">{session.name}</h3>
        ...
        {/* Top PRs */}
        <section className="space-y-6">
          <h2 className="text-2xl font-black uppercase tracking-tight flex items-center gap-3 px-2">
              ...
              topPrs.map(pr => (
                <div key={pr.exerciseId} className="p-6 flex items-center justify-between hover:bg-white/5 transition-colors group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-black uppercase tracking-tight">{pr.exerciseName}</h3>
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{pr.date?.toDate().toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-4xl font-black font-mono tracking-tighter text-white">{pr.maxWeight}</span>
